...
(function (global) {
  ...
  function lerpRate(el) {
    var v = parseFloat(
      el && el.getAttribute && el.getAttribute("data-sc-lerp"),
    );
  ...
  function mount(root, opts) {
    ...
    Array.prototype.forEach.call(
      root.querySelectorAll("[data-sc-act]"),
      function (el) {
        ...
        var act = {
          ...
          device: device,
          pinned: pinned,
          span: parseFloat(el.getAttribute("data-sc-span")) ||
            (pinned ? 1.5 : 0),
          dwell: parseFloat(el.getAttribute("data-sc-dwell")) || 0,
          clipTravel: pinned &&
            el.getAttribute("data-sc-clip-map") === "travel",
          p: 0,
          raw: 0,
          top: 0,
          height: 0,
        ...
        if (pinned) {
          act.stage = el.querySelector("[data-sc-stage]") ||
            el.querySelector(".sc-stage");
        ...
        if (cv) {
          ...
          act.seq = {
            ...
            tpl: spec[0],
            count: parseInt(spec[1], 10) || 1,
            start: parseInt(spec[2], 10) || 0,
            frames: [],
            loaded: 0,
            drawn: -1,
          };
        ...
        // cues
        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-cue]"),
          function (c) {
            var nums = (c.getAttribute("data-sc-cue") || "").trim().split(/\s+/)
              .map(parseFloat);
            ...
            var cue = {
              ...
              rOut: nums.length > 3 && !isNaN(nums[3])
                ? clamp01(nums[3])
                : null,
            ...
            if (cue.rOut === null) {
              cue.rOut = (nums.length > 2 && !isNaN(nums[2])) ? 0.3 : 0.3;
            }
        ...
        // parallax
        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-parallax]"),
          function (c) {
            act.parallax.push({
              el: c,
        ...
        // reveals
        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-reveal]"),
          function (c) {
            var nums = (c.getAttribute("data-sc-reveal-at") || "0 0.5").trim()
        ...
        // counters
        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-count]"),
          function (c) {
            var nums = (c.getAttribute("data-sc-count") || "").trim().split(
              /\s+/,
            );
            var at = (c.getAttribute("data-sc-count-at") || "0.1 0.55").trim()
              .split(/\s+/).map(parseFloat);
        ...
        if (d) {
          var rgb = parseColor(d);
          if (rgb) drifts.push({ act: act, rgb: rgb });
        }
      },
    );
    ...
    Array.prototype.forEach.call(
      ...
      function (el) {
        var W = {
          el: el,
          stage: el.querySelector("[data-sc-world]") ||
            el.querySelector(".sc-world"),
          copyLayer: el.querySelector("[data-sc-world-copy]") ||
            el.querySelector(".sc-world__copy"),
          spacer: el.querySelector("[data-sc-spacer]") ||
            el.querySelector(".sc-world__spacer"),
          seam: 0,
          segs: [],
          copies: [],
          total: 0,
        ...
        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-segment]"),
          function (s) {
            var seg = {
              el: s,
              w: parseFloat(s.getAttribute("data-sc-w")) || 1.3,
              linger: clamp(
                parseFloat(s.getAttribute("data-sc-linger")) || 0,
                0,
                0.6,
              ),
              label: s.getAttribute("data-sc-waypoint") || "",
              poster: s.querySelector("[data-sc-poster]") ||
                s.querySelector(".sc-world__poster") || s.querySelector("img"),
              c0: 0,
              c1: 0,
              local: 0,
              op: -1,
        ...
        W.segs.forEach(function (s) {
          s.c0 = run;
          run += Math.max(s.w, 0.1);
          s.c1 = run;
        });
        W.total = Math.max(run, 0.001);

        Array.prototype.forEach.call(
          el.querySelectorAll("[data-sc-copy]"),
          function (cEl) {
    ...
    if ("IntersectionObserver" in window) {
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll("[data-sc-in]"),
        function (el) {
          io.observe(el);
        },
      );
    ...
    function layout() {
      ...
      worlds.forEach(function (W) {
        if (W.spacer) {
          W.spacer.style.height = Math.round((W.total + 1) * vh) + "px";
        }
      });
      acts.forEach(function (a) {
      ...
      acts.forEach(function (a) {
        ...
        if (pos !== "sticky" && pos !== "-webkit-sticky") {
          console.warn(
            '[scrollcraft] act "' + (a.el.id || a.device) +
              '" will not pin: its stage computes ' +
              "position:" + pos +
              ", not sticky. Something is overriding .sc-stage.",
            a.stage,
          );
        }
      });
      ...
      worlds.forEach(function (W) {
        ...
        if (!W.stage) {
          console.warn(
            "[scrollcraft] worldflight has no [data-sc-world] stage; nothing will fly.",
            W.el,
          );
          return;
        }
        if (!W.spacer) {
          console.warn(
            "[scrollcraft] worldflight has no [data-sc-spacer]; the page has no scroll track.",
            W.el,
          );
        }
        ...
        if (wp !== "fixed") {
          console.warn(
            "[scrollcraft] worldflight stage computes position:" + wp +
              ", not fixed. " +
              "Something is overriding .sc-world, and the flight will scroll off screen.",
            W.stage,
          );
        }
      });
    ...
    function loadClip(V) {
      ...
      if (isMobile() && V.el.getAttribute("data-sc-src-mobile")) {
        src = V.el.getAttribute("data-sc-src-mobile");
      }
      if (!src) return;
      V.loading = true;
      fetch(src).then(function (r) {
        ...
        .then(function (blob) {
          ...
          V.el.addEventListener("loadedmetadata", function () {
            ...
            try {
              V.el.currentTime = Math.max(
                V.target * (V.el.duration || 1),
                0.001,
              );
            } catch (e) {}
            read();
    ...
    function readWorld(W) {
      ...
      for (i = 0; i < W.segs.length; i++) {
        ...
        if (s.poster && !reduce && !(s.clip && s.clip.painted) && op > 0.002) {
          s.poster.style.transform = "scale(" +
            (1.03 + local * 0.14).toFixed(4) + ")";
        }
      }
      ...
      for (var c = 0; c < W.copies.length; c++) {
        ...
        if (pr < q.from) vis = 0;
          ...
          vis = smooth((pr - q.from) / Math.max(inEnd - q.from, 0.001));
        } else if (pr <= outStart) vis = 1;
        else {vis = smooth(
            1 - (pr - outStart) / Math.max(q.to - outStart, 0.001),
          );}
        vis = clamp01(vis);
        ...
        q.el.style.transform = reduce
          ? "none"
          : "translate3d(0," + ((0.5 - wp) * 4).toFixed(2) + "vh,0)";
        var on = vis > 0.5;
        if (on !== (q.state === 1)) {
          q.state = on ? 1 : 0;
          q.el.style.pointerEvents = on ? "auto" : "none";
        }
      }
    ...
    function read() {
      ...
      for (var i = 0; i < acts.length; i++) {
        ...
        if (a.video) {
          a.video.live = a.live;
          if (a.video.ready) a.video.target = a.vp;
        }
        ...
        if (a.rail) {
          ...
          if (over > 0) {
            ...
            a.rail.style.transform = "translate3d(" +
              (-(over + extra) * a.p).toFixed(2) + "px,0,0)";
          }
        }
        ...
        if (!a.live) {
          ...
          if (a.parked !== true) {
            for (var z = 0; z < a.cues.length; z++) {
              ...
              pq.state = 0;
              if (pq.units) {
                for (var zu = 0; zu < pq.units.length; zu++) {
                  pq.units[zu].style.opacity = "0";
                }
              }
            }
            a.parked = true;
        ...
        for (var c = 0; c < a.cues.length; c++) {
          ...
          if (q.to === null) {
            ...
            if (a.p < inEnd) {
              vis = smooth((a.p - q.from) / Math.max(inEnd - q.from, 0.001));
            } else if (a.p <= outStart) vis = 1; // the plateau
            else {vis = smooth(
                1 - (a.p - outStart) / Math.max(q.to - outStart, 0.001),
              );}
          }
          vis = clamp01(vis);
          ...
          if (q.kinetic) {
            ...
            for (var u = 0; u < n; u++) {
              ...
              q.units[u].style.transform = reduce
                ? "none"
            ...
            q.el.style.transform = reduce
              ? "none"
              : "translate3d(0," + ((1 - vis) * 2.4 * q.rise).toFixed(2) +
                "vh,0)";
          }
          var on = vis > 0.5;
          if (on !== (q.state === 1)) {
        ...
        if (!reduce) {
          for (var pz = 0; pz < a.parallax.length; pz++) {
            ...
            pp.el.style.transform = "translate3d(0," +
              (pp.rate * (a.p - 0.5) * 100).toFixed(2) + "px,0)";
          }
        }
        ...
        for (var rv = 0; rv < a.reveals.length; rv++) {
          ...
          R.el.style.clipPath = R.dir === "down"
            ? "inset(" + pct + "% 0 0 0)"
            : R.dir === "left"
            ? "inset(0 " + pct + "% 0 0)"
            : R.dir === "right"
            ? "inset(0 0 0 " + pct + "%)"
            : R.dir === "iris"
            ? "circle(" + (t * 78).toFixed(2) + "% at 50% 50%)"
            : "inset(0 0 " + pct + "% 0)";
        }
      ...
      if (driftA) {
        docEl.style.setProperty(
          "--sc-canvas",
          mixColor(driftB.rgb, driftA.rgb, driftT),
        );
      }
      ...
      for (var i = 0; i < playheads.length; i++) {
        ...
        if (V.el.seeking) {
          ...
          if (!V.stuckAt) V.stuckAt = nw;
            ...
            V.stuckAt = nw;
            try {
              V.el.currentTime = V.el.currentTime + 0.001;
            } catch (e) {}
          }
          continue;
        }
    ...
    function initPointer() {
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll("[data-sc-tilt]"),
        function (el) {
          tilts.push({
            el: el,
            max: parseFloat(el.getAttribute("data-sc-tilt")) || 6,
            x: 0,
            ty: 0,
            tx: 0,
            y: 0,
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll("[data-sc-magnet]"),
        function (el) {
          magnets.push({
            el: el,
            k: parseFloat(el.getAttribute("data-sc-magnet")) || 0.3,
            x: 0,
            y: 0,
            tx: 0,
            ty: 0,
      ...
      Array.prototype.forEach.call(
        root.querySelectorAll("[data-sc-spotlight]"),
        function (el) {
          spots.push(el);
        },
      );
      ...
      addEventListener("pointermove", function (e) {
        ...
        for (var s = 0; s < spots.length; s++) {
          ...
          spots[s].style.setProperty(
            "--sc-mx",
            clamp01((e.clientX - sr.left) / sr.width).toFixed(3),
          );
          spots[s].style.setProperty(
            "--sc-my",
            clamp01((e.clientY - sr.top) / sr.height).toFixed(3),
          );
        }
      }, { passive: true });
      ...
      (function pointerTick() {
        ...
        for (var i = 0; i < tilts.length; i++) {
          ...
          if (Math.abs(T.x) > 0.001 || Math.abs(T.y) > 0.001) {
            T.el.style.transform = "perspective(1100px) rotateX(" +
              T.x.toFixed(3) + "deg) rotateY(" + T.y.toFixed(3) + "deg)";
          }
        }
        for (var m = 0; m < magnets.length; m++) {
          var M = magnets[m];
          M.x += (M.tx - M.x) * 0.12;
          M.y += (M.ty - M.y) * 0.12;
          M.el.style.transform = "translate3d(" + M.x.toFixed(2) + "px," +
            M.y.toFixed(2) + "px,0)";
        }
